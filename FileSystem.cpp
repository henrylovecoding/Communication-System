...
//---------<find files>-----------------------------------------------------------------------
/*std:: vector<std::string> findfile(std::string fileExtension, char *argv[])
{
      Directory :: setCurrentDirectory(argv[1]);
	  std::vector<std::string> currentfile = Directory::getFiles(argv[1], fileExtension);
	  for(size_t i=0; i<currentfile.size();i++)
	  {
	    std:: cout<<currentfile[i].c_str()<<"\n";
	  }
	  return currentfile;
}*/
